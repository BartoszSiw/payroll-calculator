...
public class DmsParserRO {

    private final AppLogger log = new AppLogger("DmsParserRO");
    ...
    public DmsParsedDocument parse(Document doc, String fileName) {
        ...
        if (root == null) {
            log.error("ParserRO: brak root w pliku: " + fileName);
            return out;
        }
        ...
        try {
            ...
            log.warn("ParserRO: błąd przy odczycie numeru z <numer>: " + e.getMessage());
        }
        ...
        try {
            ...
            log.warn("ParserRO: extractFromGenInfo rzucił wyjątek: " + e.getMessage());
        }
        ...
        if (!found || (out.getInvoiceShortNumber() == null && out.getInvoiceNumber() == null)) {
            try {
                ...
                log.warn("ParserRO: nie udało się wyciągnąć main number: " + e.getMessage());
            }
        }
        ...
        if (out.getInvoiceNumber() == null || out.getInvoiceNumber().isBlank()) {
            try {
                ...
                log.warn("ParserRO: fallback extractMainNumberFromDmsElement nie powiódł się: " + e.getMessage());
            }
        }

        // 4) rejestr, oddział, kasa (szukamy dokumentów typ="02")
        try {
            ...
            log.warn("ParserRO: nie udało się odczytać rejestru/oddziału: " + ex.getMessage());
        }
        ...
        // 6) kontrahent (jeśli jest typ 35)
        try {
            out.setContractor(extractContractor(doc));
        } catch (Exception e) {
            log.warn("ParserRO: nie udało się odczytać kontrahenta: " + e.getMessage());
        }

        // 7) VAT / wartości / płatności

        // 8) pozycje: RO może mieć zagnieżdżone dokumenty (np. typ 48/49) — spróbuj wyciągnąć typ="49"
        try {
            List<DmsPosition> positions = extractPositionsFromTyp49(doc);
            out.setPositions(positions == null ? Collections.emptyList() : positions);
        } catch (Exception e) {
            log.warn("ParserRO: extractPositionsFromTyp49 rzucił wyjątek: " + e.getMessage());
            out.setPositions(Collections.emptyList());
        }

        // 9) typ analizy
        out.setTypDocAnalizer("RO");

        // końcowy log
        try {
            log.info(String.format("ParserRO: END file=%s type=%s nr=%s positions=%d",
                    fileName,
                    out.getDocumentType(),
    ...
    // ---------------------------
    // extractPositionsFromTyp49 (kopiowane/zgodne z RD)
    // ---------------------------
