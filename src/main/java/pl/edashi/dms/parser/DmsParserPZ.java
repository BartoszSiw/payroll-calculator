...
/**
 * Parser dla dokumentów PZ (przyjęcia zewnętrzne).
 * Wzorzec zgodny z DmsParserDZ / DmsParserDM / DmsParserPO.
 */
public class DmsParserPZ {

    private final AppLogger log = new AppLogger("DmsParserPZ");
    ...
    public DmsParsedDocument parse(Document doc, String fileName) {
        ...
        if (root == null) {
            log.error("ParserPZ: brak root w pliku: " + fileName);
            return out;
        }
        ...
        try {
            ...
            log.warn("ParserPZ: błąd przy odczycie numeru z <numer>: " + e.getMessage());
        }
        ...
        try {
            ...
            log.warn("ParserPZ: extractFromGenInfo rzucił wyjątek: " + e.getMessage());
        }
        ...
        if (!found || (out.getInvoiceShortNumber() == null && out.getInvoiceNumber() == null)) {
            try {
                ...
                log.warn("ParserPZ: nie udało się wyciągnąć main number: " + e.getMessage());
            }
        }
        ...
        if (out.getInvoiceNumber() == null || out.getInvoiceNumber().isBlank()) {
            try {
                ...
                log.warn("ParserPZ: fallback extractMainNumberFromDmsElement nie powiódł się: " + e.getMessage());
            }
        }
        ...
        try {
            ...
            log.warn("ParserPZ: nie udało się odczytać rejestru/oddziału: " + ex.getMessage());
        }
        ...
        try {
            ...
            log.warn("ParserPZ: nie udało się odczytać kontrahenta: " + e.getMessage());
        }

        // 7) VAT / wartości / płatności

        // 8) Pozycje: PZ zwykle ma pozycje w dokument typ="03" w rozszerzone; na start zostawiamy pustą listę
        out.setPositions(Collections.emptyList());

        // 9) Typ analizy
        out.setTypDocAnalizer("PZ");

        // Końcowy log
        try {
            log.info(String.format("ParserPZ: END file=%s type=%s nr=%s positions=%d",
                    fileName,
                    out.getDocumentType(),
